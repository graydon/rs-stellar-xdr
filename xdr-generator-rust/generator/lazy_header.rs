...
#[derive(Clone)]
pub enum LazyHandle {
    Shared { buf: Arc<[u8]>, pos: u32, len: u32 },
    Inline { buf: [u8; LAZY_HANDLE_INLINE_CAPACITY], len: u8 },
}
...
impl LazyHandle {
    ...
    pub fn from_slice(buf: &[u8]) -> Self {
        if buf.len() <= LAZY_HANDLE_INLINE_CAPACITY {
            ...
            Self::Inline { buf: inline, len: buf.len() as u8 }
        } else {
    ...
    pub fn from_arc(buf: Arc<[u8]>, pos: u32, len: u32) -> Self {
        ...
        if len as usize <= LAZY_HANDLE_INLINE_CAPACITY {
            ...
            Self::Inline { buf: inline, len: len as u8 }
        } else {
    ...
    pub fn as_slice(&self) -> &[u8] {
        match self {
            Self::Shared { buf, pos, len } => {
                &buf[*pos as usize..(*pos as usize + *len as usize)]
            }
