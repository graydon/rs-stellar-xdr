...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`ScpEnvelope`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyScpEnvelope(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyScpEnvelope {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyScpEnvelope {
...
impl LazyXdr for LazyScpEnvelope {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        {
            let field_len =
                <LazyScpStatement as LazyXdr>::xdr_validate(&buf[pos as usize..], depth)?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        let mut pos: u32 = 0;
        pos += <LazyScpStatement as LazyXdr>::xdr_len(&buf[pos as usize..]);
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyScpEnvelope {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyScpEnvelope {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyScpEnvelope {
    type Error = Error;
...
impl TryFrom<&ScpEnvelope> for LazyScpEnvelope {
    type Error = Error;
    fn try_from(val: &ScpEnvelope) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyScpEnvelope> for ScpEnvelope {
    type Error = Error;
    fn try_from(lazy: &LazyScpEnvelope) -> Result<Self, Error> {
