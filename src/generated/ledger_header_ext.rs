...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`LedgerHeaderExt`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyLedgerHeaderExt(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyLedgerHeaderExt {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyLedgerHeaderExt {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyLedgerHeaderExt {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            ...
            1 => {
                let field_len = <LazyLedgerHeaderExtensionV1 as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            ...
            1 => {
                pos += <LazyLedgerHeaderExtensionV1 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyLedgerHeaderExt {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyLedgerHeaderExt {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyLedgerHeaderExt {
    type Error = Error;
...
impl LazyLedgerHeaderExt {
    ...
    #[must_use]
    pub fn as_v1(&self) -> Option<LazyLedgerHeaderExtensionV1> {
        if self.discriminant_i32() == 1 {
            Some(<LazyLedgerHeaderExtensionV1 as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&LedgerHeaderExt> for LazyLedgerHeaderExt {
    type Error = Error;
    fn try_from(val: &LedgerHeaderExt) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyLedgerHeaderExt> for LedgerHeaderExt {
    type Error = Error;
    fn try_from(lazy: &LazyLedgerHeaderExt) -> Result<Self, Error> {
