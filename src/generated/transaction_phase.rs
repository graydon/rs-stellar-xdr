...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`TransactionPhase`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyTransactionPhase(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyTransactionPhase {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyTransactionPhase {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyTransactionPhase {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            0 => {
                let field_len = <LazyVecM<LazyTxSetComponent> as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
            ...
            1 => {
                let field_len = <LazyParallelTxsComponent as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
                pos = pos.checked_add(field_len).ok_or(Error::LengthExceedsMax)?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            0 => {
                pos += <LazyVecM<LazyTxSetComponent> as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            1 => {
                pos += <LazyParallelTxsComponent as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyTransactionPhase {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyTransactionPhase {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyTransactionPhase {
    type Error = Error;
...
impl LazyTransactionPhase {
    ...
    #[must_use]
    pub fn as_v0(&self) -> Option<LazyVecM<LazyTxSetComponent>> {
        if self.discriminant_i32() == 0 {
            Some(<LazyVecM<LazyTxSetComponent> as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
    ...
    #[must_use]
    pub fn as_v1(&self) -> Option<LazyParallelTxsComponent> {
        if self.discriminant_i32() == 1 {
            Some(<LazyParallelTxsComponent as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&TransactionPhase> for LazyTransactionPhase {
    type Error = Error;
    fn try_from(val: &TransactionPhase) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyTransactionPhase> for TransactionPhase {
    type Error = Error;
    fn try_from(lazy: &LazyTransactionPhase) -> Result<Self, Error> {
