...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`StellarValueExt`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyStellarValueExt(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyStellarValueExt {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyStellarValueExt {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        ...
        match self.discriminant_i32() {
            1 => self.as_signed().cmp(&other.as_signed()),
            _ => core::cmp::Ordering::Equal,
        }
    }
}
#[cfg(feature = "alloc")]
impl LazyXdr for LazyStellarValueExt {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            ...
            1 => {
                let field_len = <LazyLedgerCloseValueSignature as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            ...
            1 => {
                pos += <LazyLedgerCloseValueSignature as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyStellarValueExt {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyStellarValueExt {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyStellarValueExt {
    type Error = Error;
...
impl LazyStellarValueExt {
    ...
    pub fn discriminant(&self) -> StellarValueType {
        ...
        StellarValueType::try_from(self.discriminant_i32()).unwrap()
    }
    /// Access arm `Signed`. Returns `Some` if the discriminant matches.
    #[must_use]
    pub fn as_signed(&self) -> Option<LazyLedgerCloseValueSignature> {
        if self.discriminant_i32() == 1 {
            Some(<LazyLedgerCloseValueSignature as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&StellarValueExt> for LazyStellarValueExt {
    type Error = Error;
    fn try_from(val: &StellarValueExt) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyStellarValueExt> for StellarValueExt {
    type Error = Error;
    fn try_from(lazy: &LazyStellarValueExt) -> Result<Self, Error> {
