...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`ContractIdPreimage`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyContractIdPreimage(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyContractIdPreimage {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyContractIdPreimage {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        ...
        match self.discriminant_i32() {
            0 => self.as_address().cmp(&other.as_address()),
            1 => self.as_asset().cmp(&other.as_asset()),
            _ => core::cmp::Ordering::Equal,
        }
    }
}
#[cfg(feature = "alloc")]
impl LazyXdr for LazyContractIdPreimage {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            0 => {
                let field_len = <LazyContractIdPreimageFromAddress as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            0 => {
                pos +=
                    <LazyContractIdPreimageFromAddress as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            1 => {
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyContractIdPreimage {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyContractIdPreimage {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyContractIdPreimage {
    type Error = Error;
...
impl LazyContractIdPreimage {
    ...
    #[must_use]
    pub fn discriminant(&self) -> ContractIdPreimageType {
        // Validated — unwrap is safe.
        ContractIdPreimageType::try_from(self.discriminant_i32()).unwrap()
    }
    /// Access arm `Address`. Returns `Some` if the discriminant matches.
    #[must_use]
    pub fn as_address(&self) -> Option<LazyContractIdPreimageFromAddress> {
        if self.discriminant_i32() == 0 {
            Some(<LazyContractIdPreimageFromAddress as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
    ...
    /// Access arm `Asset`. Returns `Some` if the discriminant matches.
    #[must_use]
    pub fn as_asset(&self) -> Option<LazyAsset> {
...
impl TryFrom<&ContractIdPreimage> for LazyContractIdPreimage {
    type Error = Error;
    fn try_from(val: &ContractIdPreimage) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyContractIdPreimage> for ContractIdPreimage {
    type Error = Error;
    fn try_from(lazy: &LazyContractIdPreimage) -> Result<Self, Error> {
