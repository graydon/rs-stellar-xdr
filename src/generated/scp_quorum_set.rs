...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`ScpQuorumSet`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyScpQuorumSet(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyScpQuorumSet {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyScpQuorumSet {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        core::cmp::Ordering::Equal
            .then_with(|| self.threshold().cmp(&other.threshold()))
            .then_with(|| self.validators().cmp(&other.validators()))
            .then_with(|| self.inner_sets().cmp(&other.inner_sets()))
    }
}
#[cfg(feature = "alloc")]
impl LazyXdr for LazyScpQuorumSet {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        {
            let field_len =
                <LazyVecM<LazyNodeId> as LazyXdr>::xdr_validate(&buf[pos as usize..], depth)?;
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyScpQuorumSet {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyScpQuorumSet {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyScpQuorumSet {
    type Error = Error;
...
impl TryFrom<&ScpQuorumSet> for LazyScpQuorumSet {
    type Error = Error;
    fn try_from(val: &ScpQuorumSet) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyScpQuorumSet> for ScpQuorumSet {
    type Error = Error;
    fn try_from(lazy: &LazyScpQuorumSet) -> Result<Self, Error> {
