...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`AuthenticatedMessage`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyAuthenticatedMessage(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyAuthenticatedMessage {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyAuthenticatedMessage {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyAuthenticatedMessage {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            0 => {
                let field_len = <LazyAuthenticatedMessageV0 as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            0 => {
                pos += <LazyAuthenticatedMessageV0 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyAuthenticatedMessage {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyAuthenticatedMessage {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyAuthenticatedMessage {
    type Error = Error;
...
impl LazyAuthenticatedMessage {
    ...
    #[must_use]
    pub fn as_v0(&self) -> Option<LazyAuthenticatedMessageV0> {
        if self.discriminant_i32() == 0 {
            Some(<LazyAuthenticatedMessageV0 as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&AuthenticatedMessage> for LazyAuthenticatedMessage {
    type Error = Error;
    fn try_from(val: &AuthenticatedMessage) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyAuthenticatedMessage> for AuthenticatedMessage {
    type Error = Error;
    fn try_from(lazy: &LazyAuthenticatedMessage) -> Result<Self, Error> {
