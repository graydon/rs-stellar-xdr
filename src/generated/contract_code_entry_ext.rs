...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`ContractCodeEntryExt`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyContractCodeEntryExt(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyContractCodeEntryExt {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyContractCodeEntryExt {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyContractCodeEntryExt {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            ...
            1 => {
                let field_len = <LazyContractCodeEntryV1 as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            ...
            1 => {
                pos += <LazyContractCodeEntryV1 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyContractCodeEntryExt {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyContractCodeEntryExt {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyContractCodeEntryExt {
    type Error = Error;
...
impl LazyContractCodeEntryExt {
    ...
    #[must_use]
    pub fn as_v1(&self) -> Option<LazyContractCodeEntryV1> {
        if self.discriminant_i32() == 1 {
            Some(<LazyContractCodeEntryV1 as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&ContractCodeEntryExt> for LazyContractCodeEntryExt {
    type Error = Error;
    fn try_from(val: &ContractCodeEntryExt) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyContractCodeEntryExt> for ContractCodeEntryExt {
    type Error = Error;
    fn try_from(lazy: &LazyContractCodeEntryExt) -> Result<Self, Error> {
