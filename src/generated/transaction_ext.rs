...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`TransactionExt`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyTransactionExt(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyTransactionExt {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyTransactionExt {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyTransactionExt {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            ...
            1 => {
                let field_len = <LazySorobanTransactionData as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            ...
            1 => {
                pos += <LazySorobanTransactionData as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyTransactionExt {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyTransactionExt {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyTransactionExt {
    type Error = Error;
...
impl LazyTransactionExt {
    ...
    #[must_use]
    pub fn as_v1(&self) -> Option<LazySorobanTransactionData> {
        if self.discriminant_i32() == 1 {
            Some(<LazySorobanTransactionData as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&TransactionExt> for LazyTransactionExt {
    type Error = Error;
    fn try_from(val: &TransactionExt) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyTransactionExt> for TransactionExt {
    type Error = Error;
    fn try_from(lazy: &LazyTransactionExt) -> Result<Self, Error> {
