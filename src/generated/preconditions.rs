...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`Preconditions`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyPreconditions(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyPreconditions {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyPreconditions {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        ...
        match self.discriminant_i32() {
            1 => self.as_time().cmp(&other.as_time()),
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyPreconditions {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            ...
            1 => {
                let field_len =
                    <LazyTimeBounds as LazyXdr>::xdr_validate(&buf[pos as usize..], depth)?;
                pos = pos.checked_add(field_len).ok_or(Error::LengthExceedsMax)?;
            }
            2 => {
                let field_len =
                    <LazyPreconditionsV2 as LazyXdr>::xdr_validate(&buf[pos as usize..], depth)?;
                pos = pos.checked_add(field_len).ok_or(Error::LengthExceedsMax)?;
            }
            _ => return Err(Error::Invalid),
        }
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            ...
            1 => {
                pos += <LazyTimeBounds as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            2 => {
                pos += <LazyPreconditionsV2 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyPreconditions {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyPreconditions {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyPreconditions {
    type Error = Error;
...
impl LazyPreconditions {
    ...
    pub fn discriminant(&self) -> PreconditionType {
        ...
        PreconditionType::try_from(self.discriminant_i32()).unwrap()
    }
    /// Access arm `Time`. Returns `Some` if the discriminant matches.
    #[must_use]
    pub fn as_time(&self) -> Option<LazyTimeBounds> {
        if self.discriminant_i32() == 1 {
            Some(<LazyTimeBounds as LazyXdr>::from_xdr_at(&self.0, 4))
        } else {
            None
        }
    }
    ...
    #[must_use]
    pub fn as_v2(&self) -> Option<LazyPreconditionsV2> {
        if self.discriminant_i32() == 2 {
            Some(<LazyPreconditionsV2 as LazyXdr>::from_xdr_at(&self.0, 4))
        } else {
            None
        }
    }
...
impl TryFrom<&Preconditions> for LazyPreconditions {
    type Error = Error;
    fn try_from(val: &Preconditions) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyPreconditions> for Preconditions {
    type Error = Error;
    fn try_from(lazy: &LazyPreconditions) -> Result<Self, Error> {
