...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`PersistedScpState`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyPersistedScpState(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyPersistedScpState {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyPersistedScpState {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyPersistedScpState {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            0 => {
                let field_len = <LazyPersistedScpStateV0 as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
            ...
            1 => {
                let field_len = <LazyPersistedScpStateV1 as LazyXdr>::xdr_validate(
                    &buf[pos as usize..],
                    depth,
                )?;
                pos = pos.checked_add(field_len).ok_or(Error::LengthExceedsMax)?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            0 => {
                pos += <LazyPersistedScpStateV0 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            1 => {
                pos += <LazyPersistedScpStateV1 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyPersistedScpState {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyPersistedScpState {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyPersistedScpState {
    type Error = Error;
...
impl LazyPersistedScpState {
    ...
    #[must_use]
    pub fn as_v0(&self) -> Option<LazyPersistedScpStateV0> {
        if self.discriminant_i32() == 0 {
            Some(<LazyPersistedScpStateV0 as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
    ...
    #[must_use]
    pub fn as_v1(&self) -> Option<LazyPersistedScpStateV1> {
        if self.discriminant_i32() == 1 {
            Some(<LazyPersistedScpStateV1 as LazyXdr>::from_xdr_at(
                &self.0, 4,
            ))
        } else {
            None
...
impl TryFrom<&PersistedScpState> for LazyPersistedScpState {
    type Error = Error;
    fn try_from(val: &PersistedScpState) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyPersistedScpState> for PersistedScpState {
    type Error = Error;
    fn try_from(lazy: &LazyPersistedScpState) -> Result<Self, Error> {
