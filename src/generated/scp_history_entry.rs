...
#[cfg(feature = "alloc")]
/// Lazy wrapper for [`ScpHistoryEntry`].
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub struct LazyScpHistoryEntry(LazyHandle);
#[cfg(feature = "alloc")]
impl PartialOrd for LazyScpHistoryEntry {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
#[cfg(feature = "alloc")]
impl Ord for LazyScpHistoryEntry {
...
#[cfg(feature = "alloc")]
impl LazyXdr for LazyScpHistoryEntry {
    fn xdr_validate(buf: &[u8], depth: u32) -> Result<u32, Error> {
        ...
        match disc {
            0 => {
                let field_len =
                    <LazyScpHistoryEntryV0 as LazyXdr>::xdr_validate(&buf[pos as usize..], depth)?;
    ...
    fn xdr_len(buf: &[u8]) -> u32 {
        ...
        match disc {
            0 => {
                pos += <LazyScpHistoryEntryV0 as LazyXdr>::xdr_len(&buf[pos as usize..]);
            }
            _ => {}
        }
        pos
...
#[cfg(feature = "alloc")]
impl From<LazyHandle> for LazyScpHistoryEntry {
    fn from(h: LazyHandle) -> Self {
        Self(h)
    }
}
#[cfg(feature = "alloc")]
impl AsRef<LazyHandle> for LazyScpHistoryEntry {
    fn as_ref(&self) -> &LazyHandle {
        &self.0
    }
}
#[cfg(feature = "alloc")]
impl TryFrom<Arc<[u8]>> for LazyScpHistoryEntry {
    type Error = Error;
...
impl LazyScpHistoryEntry {
    ...
    #[must_use]
    pub fn as_v0(&self) -> Option<LazyScpHistoryEntryV0> {
        if self.discriminant_i32() == 0 {
            Some(<LazyScpHistoryEntryV0 as LazyXdr>::from_xdr_at(&self.0, 4))
        } else {
            None
        }
    }
...
impl TryFrom<&ScpHistoryEntry> for LazyScpHistoryEntry {
    type Error = Error;
    fn try_from(val: &ScpHistoryEntry) -> Result<Self, Error> {
        let mut buf = Vec::new();
...
#[cfg(all(feature = "alloc", feature = "std"))]
impl TryFrom<&LazyScpHistoryEntry> for ScpHistoryEntry {
    type Error = Error;
    fn try_from(lazy: &LazyScpHistoryEntry) -> Result<Self, Error> {
